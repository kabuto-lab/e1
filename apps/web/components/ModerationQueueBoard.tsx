...
/**
 * Четыре колонки очереди модерации: анкеты, фото, видео/документы, отзывы.
 * Используется на /dashboard и на /dashboard/moderation.
 */
...
export type ModerationQueueBoardVariant = 'page' | 'dashboard';

export function ModerationQueueBoard({
  variant = 'page',
  className = '',
}: {
  variant?: ModerationQueueBoardVariant;
  className?: string;
}) {
  ...
  const sectionShell =
    `${t.formSection} flex min-h-0 flex-col overflow-hidden ` +
    (variant === 'page'
      ? 'max-h-[min(72vh,640px)] min-h-[200px]'
      : 'min-h-[220px] max-h-[min(65vh,560px)] xl:h-full xl:max-h-none');

  const scrollList = 'min-h-0 flex-1 space-y-2 overflow-y-auto overscroll-contain pr-1';
  ...
  const colTitle = (icon: ReactNode, title: string, count: number) => (
    <div className="mb-3 flex shrink-0 items-center justify-between gap-2 border-b border-current/10 pb-2">
      <div
        className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wide ${L ? 'text-[#1d2327]' : 'text-white'}`}
      >
        <span className={L ? 'text-[#2271b1]' : 'text-[#d4af37]'}>{icon}</span>
        {title}
      </div>
      <span
        className={`rounded-full px-2 py-0.5 font-mono text-[10px] ${L ? 'bg-[#f0f0f1] text-[#50575e]' : 'bg-white/10 text-gray-400'}`}
      >
        {count}
      </span>
    </div>
  ...
  return (
    <div className={`flex min-h-0 flex-col font-body ${className}`}>
      <div
        className={`mb-4 flex shrink-0 flex-col gap-3 sm:flex-row sm:items-center ${showBlurb ? 'sm:justify-between' : 'sm:justify-end'}`}
      >
        {showBlurb ? (
          <p className={`text-sm ${t.muted}`}>
            Анкеты на верификацию, медиа и отзывы — в одном месте. На широком экране четыре колонки, каждая со своей
            прокруткой.
          </p>
        ) : null}
        <button
          ...
          onClick={() => loadQueue()}
          disabled={loading}
          className={`inline-flex shrink-0 items-center justify-center gap-2 self-start rounded border px-4 py-2 text-sm font-semibold sm:self-auto ${t.btnSecondary}`}
        >
      ...
      {error && (
        <div
          className={`mb-4 flex shrink-0 items-start gap-2 rounded-md border px-3 py-2 text-sm ${
      ...
      {loading && profiles.length === 0 && media.length === 0 && reviews.length === 0 ? (
        ...
        <div className={gridClass}>
          <section className={sectionShell} aria-label="Анкеты на верификацию">
          ...
          <section className={sectionShell} aria-label="Фото на модерации">
          ...
          <section className={sectionShell} aria-label="Видео и документы">
          ...
          <section className={sectionShell} aria-label="Отзывы">
              ...
              {reviews.length === 0 ? (
                ...
                reviews.map((r) => (
                  ...
                  >
                    ...
                    <p
                      className={`mb-2 line-clamp-4 text-[11px] leading-snug ${L ? 'text-[#2c3338]' : 'text-gray-300'}`}
                    >
