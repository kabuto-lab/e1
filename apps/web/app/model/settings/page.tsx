  ...
  const startPolling = useCallback(() => {
    ...
    pollTimerRef.current = window.setInterval(async () => {
      ...
      if (s?.linked) {
        stopPolling();
        setLinkToken(null);
        ymGoal('telegram_linked', { role: 'model' });
      }
    }, POLL_INTERVAL_MS);
    ...
    <section className="rounded-2xl border border-white/[0.06] bg-[#141414]/80 p-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="font-body text-xs text-white/40">
              Уведомления о бронях, оплатах и новых клиентах придут в бот.
            </p>
          </div>
        </div>
      ...
      {loadingStatus ? (
        ...
        <WaitingView
          ...
          onCancel={() => { setLinkToken(null); stopPolling(); }}
      ...
      {error ? (
        <div className="mt-4 flex items-center gap-2 rounded-lg border border-red-500/20 bg-red-500/5 px-3 py-2 font-body text-xs text-red-300">
    ...
    <div className="space-y-3">
      <p className="font-body text-sm text-white/60">
        {linkToken.deepLink ? (
          ...
          <>
            Бот пока не сконфигурирован. Отправь токен вручную через{' '}
            <code className="text-[#d4af37]/80">/start link_&lt;token&gt;</code> —
