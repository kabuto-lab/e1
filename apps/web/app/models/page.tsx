...
/** Район для фильтра каталога: Москва / Подмосковье (по полю city в анкете). */
...
/** Типичные населённые пункты МО для сопоставления с полем «город» (пополняйте при необходимости). */
...
/** Категория для мобильного вида «полок» (каждая анкета только в одной полке). */
...
export default function ModelsPage() {
  ...
  /** Только мобилка (до md): сетка 2 колонки vs полки с горизонтальным скроллом */
...
function ModelCard({ model }: { model: ModelProfile }) {
  ...
  return (
    ...
    >
      <div
        className={`flex min-h-[22rem] flex-1 flex-col transition-[filter,box-shadow] duration-[650ms] ease-[cubic-bezier(0.2,0.85,0.22,1)] [perspective:min(92vw,880px)] [perspective-origin:50%_38%] ${
          flipped
        ...
        >
          ...
          >
            <div className="relative aspect-[3/4] overflow-hidden rounded-t-[var(--radius-lg)] bg-[#0a0a0a]">
              ...
              {/* Сетка выше градиента; декоративные слои z-20 — pointer-events-none, иначе перехватывают hover */}
              <div
                className="absolute inset-0 z-[15] grid grid-cols-3 grid-rows-4"
                aria-hidden
              >
