...
const Step = ({ step, title, text }: IStepProps) => (
    <div className="relative z-[1] flex flex-col items-center gap-4 px-2 text-center max-[900px]:flex-row max-[900px]:text-left">
        ...
        <div className="flex flex-col gap-1 max-[900px]:text-left">
            {title ? (
...
export const HowItWorks: FC<IProps> = ({ mode }) => {
    ...
    return (
        <div className="flex flex-col gap-12 py-24">
            ...
            {isMassageMode ? (
                <div className="relative grid grid-cols-3 gap-4 max-[900px]:grid-cols-1 max-[900px]:gap-10">
                    <div className="absolute left-[16%] right-[16%] top-[23px] h-px bg-[#d4af37]/15 max-[900px]:hidden" />

                    <Step step={1} title="Создайте профиль" text="Добавьте информацию, фотографии и направления." />
                    <Step step={2} title="Пройдите проверку" text="Профиль проверяется перед публикацией." />
                    <Step step={3} title="Получайте обращения" text="Пользователи находят профиль и переходят к общению." />
                </div>
            ) : (
